...
#ifndef SKPOKEREVALR_PROFILER_H
#define SKPOKEREVALR_PROFILER_H
...
RcppExport SEXP SKPokerEvalRProfile( void ) {
  Rcpp::Rcout << "Profiling SevenEval random access..." << std::endl;
  ...
  for (int i = 0; i < 20; ++i) {
    ...
    Rcpp::Rcout << i << ": " << profile << " ms" << std::endl;
  }
  Rcpp::Rcout << "Best random access time: " << fastest << " ms" << std::endl;
  if (fastest > 0.0) {
      double const handsPerSecond = (numberOfHands * 1000L) / fastest;
      Rcpp::Rcout << "Best random access rate: " << handsPerSecond
        << " hands/sec" << std::endl;
  }
